...
with tab_single:
    ...
    if "fundamentals_results" in st.session_state:
        ...
        st.subheader("Holdings (approximate -- see caption)")
        st.caption("Yahoo's insider/institution split doesn't map exactly to NSE's promoter/FII/DII disclosure categories.")
        h1, h2 = st.columns(2)
        h1.metric("Insider holding", f"{primary['insider_holding_pct']}%" if primary["insider_holding_pct"] is not None else "N/A")
        h2.metric("Institutional holding", f"{primary['institution_holding_pct']}%" if primary["institution_holding_pct"] is not None else "N/A")
